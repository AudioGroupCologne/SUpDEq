...
function [HRTF_L, HRTF_R] = supdeq_getArbHRTF_HUTUBS(ref,samplingGrid,mode,channel,transformCore)
...
if strcmp(transformCore,'sofia')
    ...
    if channel == 0 %Only left channel
        ...
        HRTF_L = sofia_itc(ref.HRIR.CoeffLeft, [az el]);
        HRTF_R = nan;

    elseif channel == 1 %Only right channel

        HRTF_L = nan;
        HRTF_R = sofia_itc(ref.HRIR.CoeffRight, [az el]);
...
if strcmp(transformCore,'ak')
    ...
    if channel == 0 %Only left channel
        ...
        HRTF_L = AKisht(ref.HRIR.CoeffLeft,false,[az el],'complex');
        HRTF_L = HRTF_L.';
        HRTF_R = nan;
    ...
    elseif channel == 1 %Only right channel
        ...
        HRTF_L = nan;
        HRTF_R = AKisht(ref.HRIR.CoeffRight,false,[az el],'complex');
        HRTF_R = HRTF_R.';
