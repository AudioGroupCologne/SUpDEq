...
if size(IRs,2) ~= size(viewDirections,1)
...
%%
%Write SOFA file
disp('Writing "SimpleFreeFieldHRIR" SOFA object for SSR Hornhack.');

%Get empty SOFA object
SOFAobj = SOFAgetConventions('SimpleFreeFieldHRIR');

%Get number of grid points
nPoints = size(viewDirections,1);

%Transform grid to SOFA format
viewDirections(:,2) = 90-viewDirections(:,2);
...
SOFAobj.ListenerUp_Type = 'cartesian';
SOFAobj.ListenerUp_Units = 'metre';
SOFAobj.ListenerView = [1 0 0];
SOFAobj.ListenerView = [...
            viewDirections(:,1) ... % AZ in DEG
            viewDirections(:,2) ... % EL in DEG
            sourceDistance*ones(nPoints,1)];   % nPoints source distance in m
SOFAobj.ListenerView_Type = 'spherical';
SOFAobj.ListenerView_Units = 'degree, degree, metre';       
SOFAobj.SourcePosition = [0 0 0];
SOFAobj.SourcePosition_Type = 'spherical';
SOFAobj.SourcePosition_Units = 'degree, degree, metre';
