    ...
    if ILDComp
        ...
        switch ipMethod
            case 'SH'
                ...
                %Tikhonov regularization can be applied if weights in sampling grid are erased and tikhEps ~= 0
                ILD_mc_nm = AKsht(ILD_c_pre_mc_ip, false, samplingGrid, HRTFset.Nmax, 'complex', fs, false, 'real',tikhEps);

                %Apply SH interpolation to ipSamplingGrid
                ILD_c_mc_ip = AKisht(ILD_mc_nm, false, ipSamplingGrid(:,1:2), 'complex', false, false, 'real');
            ...
            case {'NN','SARITA'} %NN and SARITA handled the same, as SARITA uses NN interpolation
                ...
                for nPoint = 1:size(ipSamplingGrid,1)
                    ...
                    for n = 1:length(idx_voronoi)
                        ILD_c_mc_ip(:,nPoint) = ILD_c_mc_ip(:,nPoint) + w_voronoi(n)*ILD_c_pre_mc_ip(:,idx_voronoi(n));
                    end
                end
            ...
            case 'Bary'
                ...
                for nPoint = 1:size(ipSamplingGrid,1)
                    ...
                    for n = 1:3 %Always 3 because of triangulization
                        ILD_c_mc_ip(:,nPoint) = ILD_c_mc_ip(:,nPoint) + bw(n)*ILD_c_pre_mc_ip(:,idx_bw(n));
                    end
                end
        end
