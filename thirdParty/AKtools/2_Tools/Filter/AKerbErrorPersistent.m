% [err, f_c, n, C] = AKerbErrorPersistent(x, target, f_lim, fs)
% Same as AKerbError but with persistent variables for speeding up multiple
% function calls. Use clear AKerbErrorPersistent to clear the persistent
...
function [err,f_c,n, C] = AKerbErrorPersistent(x,target, f_lim, fs)
...
% persistent variables for speed up
persistent C_ n_ f_c_ Nmax
if isempty(C_)
    % get ERB filter coefficients
    n_      = round(21.4*log10(0.004367*f_lim(2)+1)-21.4*log10(0.004367*f_lim(1)+1));    % number of filters
    f_c_    = ERBSpace(f_lim(1), fs/2, n_);                                               % center frequencies
    fcoefs = MakeERBFilters(fs,n_,f_lim(1));                                             % filter coefficients
    ...
    n_      = find(f_c_ <= f_lim(2), 1, 'last');
    f_c_    = f_c_(1:n_);
    fcoefs = fcoefs(1:n_,:);
    
    % estimate needed filter length (4 times the largest cycle)
    Nmax = 2^nextpow2( 1/f_c_(1) * 4 * fs );
    
    % get filter impulse responses
    C_ = zeros(Nmax,1);
    C_(1) = 1;
    C_ = ERBFilterBank(C_',fcoefs)';
    
    % make minimum phase
    C_ = AKphaseManipulation(C_, fs, 'min', 4, 0);
end
...
for k = 1:n_
    ...
    X      = repmat(abs(fft(C_(:,k), Ncur)), [1 c m]) .* (abs(fft(x,      Ncur)).^2);
    TARGET = repmat(abs(fft(C_(:,k), Ncur)), [1 c m]) .* (abs(fft(target, Ncur)).^2);
