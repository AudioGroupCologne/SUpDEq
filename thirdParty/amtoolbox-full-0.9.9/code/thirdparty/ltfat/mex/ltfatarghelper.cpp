...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
   ...
   while ( List_count(restlist) > 0 )
   {
      ...
      mxArray* argname = (mxArray*)List_shift(restlist);
      ...
      if (defkeyvals && ( mxGetFieldNumber(defkeyvals, atosRetVal) != -1))
      {
         ...
         check_keepmessage( ltfatSetStructField(keyvals, "", atosRetVal, (mxArray*)List_shift(restlist)) == 0 );
         found = 1;
      }
      ...
      if (strcmp(atosRetVal, "argimport") == 0)
      {
         ...
         mxArray* first = (mxArray*)List_shift(restlist);
         mxArray* second = (mxArray*)List_shift(restlist);
...
error:
   if (NULL != restlist)
   {
      ...
      {
         mxDestroyArray((mxArray*)(cur->value));
      }
